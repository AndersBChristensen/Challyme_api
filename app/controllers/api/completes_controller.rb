class Api::CompletesController < ApplicationController
  ...
  def showAllActionForUser
    ...
    actions = Invite.select('challenges.title as challengetitle','actions.id as action_id', 'actions.name as actionname', 'task_dates.date as taskdate', 'task_dates.id as taskdate_id', 'invites.user_id as user', 'IF(completed.invite_id=invites.id AND completed.task_date_id=task_dates.id) as completed', 'invites.id')
